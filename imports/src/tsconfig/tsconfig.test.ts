...
function makeTempMonorepo(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "saf-tsconfig-"));
  tempDirs.push(root);
...
describe("saf-imports tsconfig", () => {
  ...
  describe("graph (workspace dependencies)", () => {
          ...
          JSON.stringify(
            {
              ...
              ...(compositionRoot
                ? {
                    safImports: {
                      compositionRoot: { includeSiblingPackages: true },
                    },
                  }
                : {}),
              dependencies:
