...
const routeStub = path.join(specRoot, "routes", "__group-name__", "__target-name__.yaml");
...
/**
 * Ported from `openapi/workflows/add-route.ts` — same templates, same
...
>({
  ...
  inputSchema: {
    ...
    properties: {
      path: {
        type: "string",
        description: "The file path for the route (e.g., './routes/recipes/list.yaml')",
      },
      urlPath: {
        type: "string",
        description: "The URL path for the route (e.g., '/recipes' or '/recipes/{id}')",
      },
      method: {
        type: "string",
  ...
  steps: [
    step<CopyStepInput, OpenApiRouteWorkflowContext>("copy", runCopyStep, ({ context }) => ({
      templateFiles: {
        route: routeStub,
        openapi: openapiLive,
      },
    ...
    step<TransformFileStepInput, OpenApiRouteWorkflowContext>(
      "transform-file",
      runTransformFileStep,
      ({ context }) => ({
    ...
    step<CommandStepInput, OpenApiRouteWorkflowContext>("command", runCommandStep, () => ({
      command: "npm",
      args: ["run", "generate"],
    })),
    ...
    step<CommandStepInput, OpenApiRouteWorkflowContext>("command", runCommandStep, () => ({
      ...
      errorPrompt: `JSON success responses must be flat objects keyed by resource name — not a bare business object, array, or $ref at the root. Fix the route YAML (see @saflib/openapi docs/02-api-design.md). Do not add new allowlist entries for new routes.`,
    })),
...
export function mergeOpenApiRoute(content: string): string {
  ...
  const areaEnd = lines.findIndex((l, i) => i > areaStart && l.includes("END WORKFLOW AREA"));
  ...
  return [...lines.slice(0, areaStart + 1), ...newAreaLines, ...lines.slice(areaEnd)].join("\n");
}

export default OpenApiRouteWorkflowDefinition;
