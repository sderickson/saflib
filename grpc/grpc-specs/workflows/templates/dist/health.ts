...
export abstract class UnimplementedHealthService {
    static definition = {
        HealthCheck: {
            path: "/template.file.v1.Health/HealthCheck",
            requestStream: false,
...
export class HealthClient extends grpc_1.makeGenericClientConstructor(UnimplementedHealthService.definition, "Health", {}) {
    ...
    HealthCheck: GrpcPromiseServiceInterface<HealthCheckRequest, HealthCheckResponse> = (message: HealthCheckRequest, metadata?: grpc_1.Metadata | grpc_1.CallOptions, options?: grpc_1.CallOptions): Promise<HealthCheckResponse> => { if (!metadata) {
        metadata = new grpc_1.Metadata;
    } if (!options) {
        options = {};
    } return new Promise((resolve, reject) => super.HealthCheck(message, metadata, options, (error: grpc_1.ServiceError, response: HealthCheckResponse) => {
        if (error) {
            reject(error);
        }
        else {
