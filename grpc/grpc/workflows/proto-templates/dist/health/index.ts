...
export abstract class Unimplemented__ServiceName__HealthService {
  static definition = {
    HealthCheck: {
      ...
      requestSerialize: (
        message: dependency_1.__ServiceName__HealthCheckRequest,
      ) => Buffer.from(message.serialize()),
      requestDeserialize: (bytes: Buffer) =>
        dependency_1.__ServiceName__HealthCheckRequest.deserialize(
          new Uint8Array(bytes),
        ),
      responseSerialize: (
        message: dependency_1.__ServiceName__HealthCheckResponse,
      ) => Buffer.from(message.serialize()),
      responseDeserialize: (bytes: Buffer) =>
        dependency_1.__ServiceName__HealthCheckResponse.deserialize(
          new Uint8Array(bytes),
        ),
    },
  };
  ...
  abstract HealthCheck(
    call: grpc_1.ServerUnaryCall<
      dependency_1.__ServiceName__HealthCheckRequest,
      dependency_1.__ServiceName__HealthCheckResponse
    >,
    callback: grpc_1.sendUnaryData<dependency_1.__ServiceName__HealthCheckResponse>,
  ): void;
}
export class __ServiceName__HealthClient extends grpc_1.makeGenericClientConstructor(
  Unimplemented__ServiceName__HealthService.definition,
  "__ServiceName__Health",
  {},
) {
  constructor(
    address: string,
  ...
  > = (
    ...
    options?: grpc_1.CallOptions,
  ): Promise<dependency_1.__ServiceName__HealthCheckResponse> => {
    if (!metadata) {
    ...
    return new Promise((resolve, reject) =>
      super.HealthCheck(
        ...
        (
          ...
          response: dependency_1.__ServiceName__HealthCheckResponse,
        ) => {
          if (error) {
            reject(error);
          } else {
