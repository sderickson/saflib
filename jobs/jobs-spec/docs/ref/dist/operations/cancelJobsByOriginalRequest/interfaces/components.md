...
Wire form of a queued API-call job; field names match the `job` row and nested JSON. Omits internal columns (`heartbeat_at`, `updated_at`). Authority is the discriminated evidence grant without the raw enqueue assertion token (detail endpoints may return that separately as `authority_assertion`).
...
##### job.enqueued\_by\_operation\_id
