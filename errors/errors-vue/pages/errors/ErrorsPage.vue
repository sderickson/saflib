  ...
  <ContentWidth variant="full" class="errors-page">
    <div class="d-flex flex-wrap align-center ga-3 mb-4">
      ...
      <v-btn
        size="small"
        variant="tonal"
        :loading="errorsQuery.isFetching.value"
        @click="errorsQuery.refetch()"
      >
        {{ strings.refresh }}
      </v-btn>
    </div>
    ...
    <div class="d-flex flex-wrap ga-2 mb-4">
      <v-select
        v-model="kindFilter"
        :label="strings.kind_label"
        :items="strings.kindOptions"
        density="compact"
        hide-details
        clearable
      ...
      <v-text-field
        ...
        :label="strings.source_filter_label"
        type="search"
        density="compact"
        hide-details
        clearable
    ...
    <v-table density="compact" class="errors-table">
      <thead>
        <tr>
          ...
          <th>{{ strings.columns.kind }}</th>
          <th>{{ strings.columns.source }}</th>
          <th>{{ strings.columns.message }}</th>
          <th>{{ strings.columns.timestamp }}</th>
          <th>{{ strings.columns.details }}</th>
        </tr>
      </thead>
      <tbody>
...
import { useErrorsPageLoader } from "./Errors.loader.ts";

const { errorsQuery } = useErrorsPageLoader();
