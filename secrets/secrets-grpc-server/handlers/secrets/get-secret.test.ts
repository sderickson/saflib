import { describe, it, beforeEach, expect, vi, afterEach, assert } from "vitest";
import {
  makeContext,
...
import { GetSecretError, GetSecretRequest, RegisterTokenRequest } from "@saflib/secrets-grpc-proto";
import { SafAuth } from "@saflib/grpc";
import type { DbKey } from "@saflib/drizzle";
import { accessRequestQueries, secretQueries, serviceTokenQueries } from "@saflib/secrets-db";
...
describe("handleGetSecret", () => {
  ...
  it("returns secrets if everything is in order", async () => {
    ...
    const notRegisteredResult = await handleGetSecret(new GetSecretRequest({
      secret_name,
      token,
    }));
    expect(notRegisteredResult.result).toBe("error");
    expect(notRegisteredResult.error).toBe(GetSecretError.GET_SECRET_INVALID_TOKEN);
    ...
    const notApprovedResult = await handleGetSecret(new GetSecretRequest({
      secret_name,
      token,
    }));
    expect(notApprovedResult.result).toBe("error");
    expect(notApprovedResult.error).toBe(GetSecretError.GET_SECRET_TOKEN_NOT_APPROVED);
    ...
    const { result: approvedToken } = await serviceTokenQueries.updateApproval(dbKey, {
      id: tokenId,
      approved: true,
    ...
    const notGrantedResult = await handleGetSecret(new GetSecretRequest({
      secret_name,
      token,
    }));
    expect(notGrantedResult.result).toBe("error");
    expect(notGrantedResult.error).toBe(GetSecretError.GET_SECRET_ACCESS_NOT_GRANTED);

    // ask again, since it should be created already
    const notGrantedResult2 = await handleGetSecret(new GetSecretRequest({
      secret_name,
      token,
    }));
    expect(notGrantedResult2.result).toBe("error");
    expect(notGrantedResult2.error).toBe(GetSecretError.GET_SECRET_ACCESS_NOT_GRANTED);
    ...
    const { result: approvedAccessRequest } = await accessRequestQueries.updateStatus(dbKey, {
      id: accessRequestId,
      status: "granted",
    ...
    const notFoundResult = await handleGetSecret(new GetSecretRequest({
      secret_name,
      token,
    }));
    ...
    const activeSecretResult = await handleGetSecret(new GetSecretRequest({
      secret_name,
      token,
    }));
    ...
    const inactiveSecretResult = await handleGetSecret(new GetSecretRequest({
      secret_name,
      token,
    }));
    expect(inactiveSecretResult.result).toBe("error");
    expect(inactiveSecretResult.error).toBe(GetSecretError.GET_SECRET_NOT_ACTIVE);
