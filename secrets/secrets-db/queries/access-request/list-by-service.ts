...
export type ListByServiceError = never;

export const listByService = queryWrapper(
  async (
    dbKey: DbKey,
    serviceName: string,
  ): Promise<ReturnsError<AccessRequestEntity[], ListByServiceError>> => {
    const db = secretsDbManager.get(dbKey)!;
    const result = await db
      .select()
      .from(accessRequestTable)
      .where(eq(accessRequestTable.serviceName, serviceName));
