...
export abstract class UnimplementedSecretsService {
    static definition = {
        ...
        RegisterToken: {
            ...
            requestStream: false,
            responseStream: false,
            requestSerialize: (message: dependency_2.RegisterTokenRequest) => Buffer.from(message.serialize()),
            requestDeserialize: (bytes: Buffer) => dependency_2.RegisterTokenRequest.deserialize(new Uint8Array(bytes)),
            responseSerialize: (message: dependency_2.RegisterTokenResponse) => Buffer.from(message.serialize()),
            responseDeserialize: (bytes: Buffer) => dependency_2.RegisterTokenResponse.deserialize(new Uint8Array(bytes))
        }
    };
    ...
    abstract RegisterToken(call: grpc_1.ServerUnaryCall<dependency_2.RegisterTokenRequest, dependency_2.RegisterTokenResponse>, callback: grpc_1.sendUnaryData<dependency_2.RegisterTokenResponse>): void;
}
export class SecretsClient extends grpc_1.makeGenericClientConstructor(UnimplementedSecretsService.definition, "Secrets", {}) {
    ...
    RegisterToken: GrpcPromiseServiceInterface<dependency_2.RegisterTokenRequest, dependency_2.RegisterTokenResponse> = (message: dependency_2.RegisterTokenRequest, metadata?: grpc_1.Metadata | grpc_1.CallOptions, options?: grpc_1.CallOptions): Promise<dependency_2.RegisterTokenResponse> => { if (!metadata) {
        metadata = new grpc_1.Metadata;
    } if (!options) {
        options = {};
    } return new Promise((resolve, reject) => super.RegisterToken(message, metadata, options, (error: grpc_1.ServiceError, response: dependency_2.RegisterTokenResponse) => {
        if (error) {
            reject(error);
        }
        else {
