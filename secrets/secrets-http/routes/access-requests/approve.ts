...
export const approveAccessRequestHandler = createHandler(
  async (req, res) => {
    const ctx = secretsServiceStorage.getStore()!;
    const data: SecretsServiceRequestBody["approveAccessRequest"] = req.body || {};
    const id = req.params.id as string;

    // Get the access request first to check if it exists
    const { result: existingRequest, error: getError } = await accessRequest.getById(ctx.secretsDbKey, id);
