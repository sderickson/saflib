...
## Keep Import Graphs Small and Enforceable

Large static import graphs slow tests, inflate bundles, and hide coupling. SAF treats import graphs as a
**measurable, CI-gated** property of each package.

**Rules:**

- **Deep imports** — `@scope/pkg/subpath`, not package roots or deleted barrels (`@pathclerk/daemon-sdk`,
  `@pathclerk/daemon-clients-common` root, etc.).
- **Hybrid `exports`** — wildcard patterns in `package.json` for large packages; `exportsAliases` for
  exceptions. Validated with `saf-imports exports check`.
...
- **`importBudget`** — optional per-package limits; default CI uses **baseline diff** instead.

**Tooling:** [`@saflib/imports`](./imports/docs/01-overview.md) — `measure`, `why`, `baseline diff`,
`spa`, `exports`, `side-effects scan`. Package cost table:
`node saflib/imports/scripts/generate-import-budgets.mjs`. CI: `npm run import-graph:check`. See
[import graph CI docs](./imports/docs/02-ci.md) and [scaffold defaults](./imports/docs/05-scaffold.md).

**Benefits:**

- Tests stay fast as the monorepo grows; graph regressions fail PRs instead of silently accumulating.
- Bundle size stays bounded (`sideEffects` + shell budgets).
- New scaffolded packages inherit the same constraints.

**Example application:**

- PathClerk M0–M9 [import graph scalability plan](https://github.com/pathclerk/pathclerk/blob/main/daemon/plans/notes/2026-08-07-test-import-graph/test-import-graph.spec.md) — measurement, decomposition, harnesses, permanent policy.
