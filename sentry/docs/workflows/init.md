...
Or from `@saflib/sentry`:

```bash
cd saflib/sentry && npm exec saf-workflow kickoff sentry/init
```
...
- Prompt the agent with concrete areas to edit (GitHub Actions, Vite/Docker, `@saflib/sentry/vite-build`, secrets).
