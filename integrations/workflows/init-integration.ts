...
5. Implement the **mock client** in \`client.mocks.ts\` (already imported). Put all mock data and mock method implementations there — keep \`client.mocks.ts\` **SDK-free** (\`import type\` only from the vendor package). Tests import mocks via \`@<package>/mocks\`, not the root barrel.
6. For SDK-backed integrations, split real SDK wiring into \`client.real.ts\` and wire production configure from \`dependencies.integrations.ts\` (see existing anthropic/stripe/gcloud packages).
