...
1. **Load PostHog** — pick one:
   - **SPA (recommended):** in `main.ts`, `import { initPostHogIfConfigured } from "@saflib/posthog-client/init"` and call `initPostHogIfConfigured()` when `VITE_POSTHOG_PROJECT_API_KEY` is set at build time.
   - **Vite HTML plugin:** use `makePosthogScriptTag()` from `@saflib/posthog-client/html` in your build config (server-side env via `@saflib/env`).
