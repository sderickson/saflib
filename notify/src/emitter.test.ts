...
function event(
  overrides: Partial<ChangeEvent> & Pick<ChangeEvent, "channel_id">,
): ChangeEvent {
  return {
...
describe("InProcessChangeEmitter", () => {
  test("delivers published events to channel subscribers", () => {
    const emitter = new InProcessChangeEmitter();
    const received: ChangeEventWithId[] = [];
    emitter.subscribe("org:a", (e) => received.push(e));

    emitter.publish(event({ channel_id: "org:a" }));
    emitter.publish(event({ channel_id: "org:b", operation_id: "other" }));
  ...
  test("unsubscribe stops delivery", () => {
    ...
    emitter.publish(event({ channel_id: "org:a" }));
    unsubscribe();
    emitter.publish(event({ channel_id: "org:a", operation_id: "second" }));
  ...
  test("getEventsAfter replays buffered events after Last-Event-ID", () => {
    const emitter = new InProcessChangeEmitter();
    emitter.publish(event({ channel_id: "org:a", operation_id: "a" }));
    emitter.publish(event({ channel_id: "org:a", operation_id: "b" }));
    emitter.publish(event({ channel_id: "org:a", operation_id: "c" }));
  ...
  test("ring buffer drops oldest when over capacity", () => {
    const emitter = new InProcessChangeEmitter({ maxEventsPerChannel: 3 });
    for (let i = 0; i < 5; i++) {
      emitter.publish(event({ channel_id: "org:a", operation_id: `op-${i}` }));
    }
  ...
  test("ring buffer drops events older than max age", () => {
    ...
    emitter.publish(event({ channel_id: "org:a", operation_id: "old" }));
    now += 500;
    emitter.publish(event({ channel_id: "org:a", operation_id: "mid" }));
    now += 600;
    emitter.publish(event({ channel_id: "org:a", operation_id: "new" }));
  ...
  test("isolates buffers and subscribers by channelId", () => {
    const emitter = new InProcessChangeEmitter();
    ...
    emitter.publish(event({ channel_id: "org:a" }));
    emitter.publish(event({ channel_id: "org:b", operation_id: "b-op" }));
  ...
  test("listener errors do not prevent other subscribers", () => {
    ...
    expect(() => emitter.publish(event({ channel_id: "org:a" }))).not.toThrow();
