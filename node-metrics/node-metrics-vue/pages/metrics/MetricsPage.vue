  ...
  <ContentWidth variant="full" class="metrics-page">
    <div class="d-flex flex-wrap align-center ga-3 mb-4">
      ...
      <v-btn
        size="small"
        variant="tonal"
        :loading="metricsQuery.isFetching.value"
    ...
    <div v-if="selectedName" class="d-flex flex-wrap ga-2 mb-4">
      ...
      <v-select
        ...
        :label="strings.add_label_filter"
        density="compact"
        hide-details
        clearable
        style="min-width: 180px; max-width: 240px"
    ...
    >
      ...
      <v-table density="compact" class="metrics-table">
        <thead>
          <tr>
            <th>{{ strings.columns.metric }}</th>
            <th class="value-col">{{ strings.columns.value }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in group.rows" :key="row.key">
            <td class="metric-cell">
              <div class="metric-head">
                ...
                <v-chip size="x-small" variant="tonal" class="metric-type-pill">
                  {{ row.type }}
                </v-chip>
              </div>
              <div
    ...
    <p
      v-if="displayRowCount === 0 && !metricsQuery.isLoading.value"
...
import { useMetricsPageLoader } from "./Metrics.loader.ts";
import {
...
const { metricsQuery } = useMetricsPageLoader();
...
watch(selectedName, (name) => {
  if (!name) {
    ...
    splitLabel.value &&
    !splitLabelOptions.value.includes(splitLabel.value)
  ) {
    splitLabel.value = null;
  }
