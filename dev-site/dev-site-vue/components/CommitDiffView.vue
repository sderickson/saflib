...
import { formatLoc } from "../format-loc.ts";

type CommitDiff = DevSiteResponseBody["diffCommits"][200]["commit_diff"];

const props = defineProps<{ diff: CommitDiff }>();
...
const exportRows = computed(() => [
  ...props.diff.exports.added.map((e) => ({ ...e, change: "added" as const })),
  ...props.diff.exports.removed.map((e) => ({ ...e, change: "removed" as const })),
]);

const testRows = computed(() => [
  ...props.diff.test_cases.added.map((t) => ({ ...t, change: "added" as const })),
  ...props.diff.test_cases.removed.map((t) => ({ ...t, change: "removed" as const })),
]);
